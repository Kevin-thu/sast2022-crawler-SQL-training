...
class ZhihuCrawler:
    ...
    def watch(self, top=None):
        ...
        while True:
            ...
            self.sleep("interval_between_board")
...
"""
    ...
    def get_board(self) -> list:
        ...
        resp = requests.get("https://www.zhihu.com/hot", headers=self.settings['headers'])
        assert resp.status_code == 200
        soup = BS(resp.text, features="html.parser")
        HotItems = soup.find_all("section", class_="HotItem")
        HotList = []
        for HotItem in HotItems:
            HotInfo = {}
            HotInfo['title'] = HotItem.find("h2", class_="HotItem-title").text if HotItem.find("h2", class_="HotItem-title") is not None else None
            HotInfo['heat'] = HotItem.find("div", class_="HotItem-metrics HotItem-metrics--bottom").text[:-len("\u200b分享")] if HotItem.find("div", class_="HotItem-metrics HotItem-metrics--bottom") is not None else "0"
            HotInfo['excerpt'] = HotItem.find("p", class_="HotItem-excerpt").text if HotItem.find("p", class_="HotItem-excerpt") is not None else None
            HotInfo['url'] = HotItem.find("a")['href']
            HotInfo['qid'] = int(HotInfo['url'][len("https://www.zhihu.com/question/"):])
            HotList.append(HotInfo)
        return HotList
    ...
    def get_question(self, qid: int) -> dict:
        ...
        resp = requests.get("https://www.zhihu.com/question/" + str(qid), headers=self.settings['headers'])
        assert resp.status_code == 200
        soup = BS(resp.text, features="html.parser")
        data = soup.find("script", id="js-initialData").text
        data = json.loads(data)['initialState']['entities']['questions'][str(qid)]
        QuesInfo = {}
        QuesInfo['created'] = data['created']
        QuesInfo['followerCount'] = data['followerCount']
        QuesInfo['visitCount'] = data['visitCount']
        QuesInfo['answerCount'] = data['answerCount']
        QuesInfo['raw'] = data['detail']
        QuesInfo['hit_at'] = time.time()
        return QuesInfo
